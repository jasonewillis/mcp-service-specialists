# IT Specialist Agent - Fed Job Advisor MCP Agent

**Agent Type**: Role-Based Technical Agent  
**Domain**: Federal IT Specialist Career Analysis (Series 2210)  
**Endpoint**: `http://localhost:8001/agents/it-specialist/analyze`  
**Status**: Active  
...
### Primary Function
Provides comprehensive federal IT specialist career analysis, including general IT skills assessment, systems administration evaluation, and troubleshooting capabilities for broad IT Specialist positions (Series 2210).

### Federal Expertise
- **Job Series**: 2210 (IT Specialist), 0334 (Computer Specialist), 1550 (Computer Sciences)
- **Domain Knowledge**: Federal IT standards, government systems administration, help desk operations
...
### API Endpoint
```bash
POST http://localhost:8001/agents/it-specialist/analyze
```
...
```json
{
  ...
  "context": {
    "skills": ["Windows Server", "Active Directory", "Network Administration", "Help Desk", "ITIL"],
    "experience": "IT support for federal agency, 4 years systems administration",
...
async def analyze_with_it_specialist_agent(task_data):
    async with httpx.AsyncClient() as client:
        response = await client.post(
            "http://localhost:8001/agents/it-specialist/analyze",
            json={
                "user_id": "user123",
...
# Example usage
result = await analyze_with_it_specialist_agent({
    "skills": ["Windows Server", "Active Directory", "Network Administration", "Help Desk", "ITIL"],
    "experience": "IT support for federal agency, 4 years systems administration"
})
```
...
# 1. Claude Code identifies task requiring it specialist agent expertise
# 2. Routes to IT Specialist Agent for specialized federal analysis
...
### Use Cases
1. **Federal Expertise**: Specialized analysis for federal it specialist career analysis (series 2210)
...
- **Documentation Output**: Generates analysis in `_Management/_PM/_Tasks/IT-SPECIALIST_ANALYSIS.md`
...
### Task Routing Logic
```typescript
// When Claude Code uses IT Specialist Agent
if (task.domain.includes("it-specialist") || 
    task.type === "it_specialist_analysis" ||
    task.requiresFederalExpertise) {
    return await callMCPAgent("it-specialist", taskContext);
}
```
...
```python
# Feedback loop for IT Specialist Agent optimization
async def provide_agent_feedback(implementation_result):
    ...
    await submit_feedback("http://localhost:8001/feedback/it-specialist", feedback)
```
...
### Test Scenarios
```python
# IT Specialist Agent validation test cases
test_cases = [
    {
...
### OPM 2210 IT Specialist Expertise
...
**© 2025 Fed Job Advisor IT Specialist Agent - Optimized for Ultimate Workflow Integration**