# Database Administrator Agent - Fed Job Advisor MCP Agent

**Agent Type**: Role-Based Technical Agent  
**Domain**: Federal Database Administrator Career Analysis (Series 2210/0334)  
**Endpoint**: `http://localhost:8001/agents/database-admin/analyze`  
**Status**: Active  
...
### Primary Function
Provides comprehensive federal database administrator career analysis, including database platform matching, security clearance guidance, and performance tuning assessment for DBA positions (Series 2210/0334).

### Federal Expertise
- **Job Series**: 2210 (IT Specialist), 0334 (Computer Specialist), 1550 (Computer Sciences)
- **Domain Knowledge**: Federal database security standards, FISMA compliance, enterprise database management
...
### API Endpoint
```bash
POST http://localhost:8001/agents/database-admin/analyze
```
...
```json
{
  ...
  "context": {
    "skills": ["SQL", "Oracle", "PostgreSQL", "Database Security", "Performance Tuning"],
    "experience": "Database architecture for federal systems, 7 years",
...
async def analyze_with_database_admin_agent(task_data):
    async with httpx.AsyncClient() as client:
        response = await client.post(
            "http://localhost:8001/agents/database-admin/analyze",
            json={
                "user_id": "user123",
...
# Example usage
result = await analyze_with_database_admin_agent({
    "skills": ["SQL", "Oracle", "PostgreSQL", "Database Security", "Performance Tuning"],
    "experience": "Database architecture for federal systems, 7 years"
})
```
...
# 1. Claude Code identifies task requiring database administrator agent expertise
# 2. Routes to Database Administrator Agent for specialized federal analysis
...
### Use Cases
1. **Federal Expertise**: Specialized analysis for federal database administrator career analysis (series 2210/0334)
...
- **Documentation Output**: Generates analysis in `_Management/_PM/_Tasks/DATABASE-ADMIN_ANALYSIS.md`
...
### Task Routing Logic
```typescript
// When Claude Code uses Database Administrator Agent
if (task.domain.includes("database-admin") || 
    task.type === "database_admin_analysis" ||
    task.requiresFederalExpertise) {
    return await callMCPAgent("database-admin", taskContext);
}
```
...
```python
# Feedback loop for Database Administrator Agent optimization
async def provide_agent_feedback(implementation_result):
    ...
    await submit_feedback("http://localhost:8001/feedback/database-admin", feedback)
```
...
### Test Scenarios
```python
# Database Administrator Agent validation test cases
test_cases = [
    {
...
### OPM 2210/0334 Series Expertise
...
**© 2025 Fed Job Advisor Database Administrator Agent - Optimized for Ultimate Workflow Integration**